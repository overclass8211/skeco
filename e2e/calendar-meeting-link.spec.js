...
test('영업캘린더 — 회의록 연동 링크 + 계획 정리 + 참조 보존', async ({ page }) => {
  ...
  // 3) 일정 수정 모달: [연결된 회의록] 행 + 회의록 제목 링크, 계획에 raw meeting:N 없음
  await expect(page.locator('#cal-linked-meeting-row .form-label')).toHaveText('연결된 회의록');
  const link = page.locator('#cal-nav-meeting');
  await expect(link).toBeVisible({ timeout: 5000 });
  await expect(link).toHaveAttribute('data-meeting-id', String(setup.meetingId));
  // 링크 텍스트가 회의록 제목으로 비동기 채워짐
  await expect(page.locator('#cal-linked-meeting-title')).toHaveText(`E2E 회의록 ${cust}`, {
    timeout: 5000,
  });
