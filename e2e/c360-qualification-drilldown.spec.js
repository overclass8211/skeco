...
test('360 공급 자격 — 샘플·품질 행 클릭 시 세부 드릴다운 모달', async ({ page }) => {
  ...
  // ── 자격 요약 스트립(슬림 UX) — 상태 타일 노출 ──
  await expect(page.locator('.c360-qtile').first()).toBeVisible({ timeout: 10000 });
  expect(await page.locator('.c360-qtile').count()).toBeGreaterThanOrEqual(3);
