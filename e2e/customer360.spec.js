...
const DETAIL = {
  ...
  data: {
    ...
    lifecycle: {
      materials: [
        { id: 11, material_name: '식각가스 C4F6 · Fab12', business_type: '식각가스', fab_line: '평택 P3', lifecycle_stage: 'specin', lifecycle_label: 'Mass-prod Readiness', lifecycle_index: 6, current_gate: 'MRP', current_gate_label: 'Mass-prod Readiness', gates: [
          { gate_key: 'MRD', gate_label: 'Market Requirement', status: 'done', target_date: '2026-01-15', actual_date: '2026-01-10', late: false },
          { gate_key: 'CRP', gate_label: 'Customer Requirement', status: 'done', target_date: '2026-02-15', actual_date: '2026-02-12', late: false },
          { gate_key: 'DOE', gate_label: 'Design of Experiments', status: 'done', target_date: '2026-03-20', actual_date: '2026-03-18', late: false },
          { gate_key: 'PROTO', gate_label: 'Prototype', status: 'done', target_date: '2026-04-30', actual_date: '2026-04-28', late: false },
          { gate_key: 'SMALL', gate_label: 'Small-lot', status: 'done', target_date: '2026-06-20', actual_date: '2026-06-18', late: false },
          { gate_key: 'GALLON', gate_label: 'Gallon-scale', status: 'done', target_date: '2026-07-25', actual_date: '2026-07-22', late: false },
          { gate_key: 'MRP', gate_label: 'Mass-prod Readiness', status: 'in_progress', target_date: '2026-09-30', actual_date: null, late: false },
          { gate_key: 'MP', gate_label: 'Mass Production', status: 'pending', target_date: '2026-11-30', actual_date: null, late: false },
        ], expected_mp_date: '2026-08-01', monthly_demand: 2000, demand_unit: 'kg', win_probability: 80, quarter_demand: 6000, quarter_capacity: 5000, quarter_expected_order: 4000000000, capa_short: true, open_quality: 1, linked_deal_count: 1, primary_lead_id: 555, linked_deals: [{ id: 555, project_name: 'Fab12 식각가스 공급', stage: 'won', stage_label: '수주 완료', prob: 100, expected_amount: 8800000000 }] },
      ],
...
test('고객·제품 360뷰 — 라이프사이클 보드 + 수요·생산·수주 + 탭', async ({ page }) => {
  ...
  // 게이트=단일 소스: 현재 게이트(.ss-now)는 게이트 키(MRP) + '현재' 칩 표시
  await expect(page.locator('.lc-card .ss-now')).toContainText('MRP');
  await expect(page.locator('.lc-card .ss-now')).toContainText('현재');
  await expect(page.locator('.lc-card')).toContainText('CAPA 부족');

  // 분기 공급 리스크 블록은 제거 → '공급 충족' KPI로 통합. CAPA 부족 시 부족 표시 + AI 진단 진입(#c360-capa-box)
  await expect(page.locator('.c360-kpi', { hasText: '공급 충족' })).toContainText('부족');
  await expect(page.locator('#c360-capa-box')).toHaveCount(1);

  // AI 액션 (카드 v2)
  await expect(page.locator('.c360-act2')).toContainText('CAPA 재검토');

  // 영업·매출 탭 = 영업딜 + 포캐스트 + 계약/매출/수금 (통합)
  ...
  // 월별 추이 콤보 차트(canvas) + 합계 표 동시 표시 + 숫자 헤더 우측 정렬
  await expect(page.locator('#c360-fc-chart')).toBeVisible({ timeout: 5000 });
  await expect(page.locator('#c360-fc table.data-table').first()).toBeVisible();
  await expect(page.locator('#c360-fc th.text-right').first()).toHaveCSS('text-align', 'right');
  // 계약/매출/수금 섹션은 제거됨 — 노출되지 않아야 함
  await expect(page.locator('#c360-rev')).toHaveCount(0);
